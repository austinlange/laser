...
DataObject.searchByField = function(object, field, search, callback) {
	if (!field || !search) {
		return callback(new Error("Invalid parameters to searchByField"));
	}

	var query = util.format("SELECT * FROM %s WHERE lower(%s) LIKE '%%%s%%'", new object().tableName, field, search);
